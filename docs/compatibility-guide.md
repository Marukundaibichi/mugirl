...
- Facial Animation：条件目录加载；只在雪牛娘种族 Def 存在时追加 comp 和动画。新增或调整 FA 内容前先看 `facial-animation-guide.md`，尤其是 `raceName=MooGirl` 专属池、`normal_MooGirl` 底座动画、贴图回退和 `FA/Eyes/Common` mask 决策。
