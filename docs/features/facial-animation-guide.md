...
- 427 张改动贴图的原图、缩放结果、全部 847 张贴图的 SHA256 清单和执行脚本保存在 `DevData/Backups/FA-Resize512-20260921/`。后续新增 FA 贴图也使用 512×512；本节尺寸约定取代此前导入时保留原始尺寸的要求。
...
- 9 张 `Lids/Normal`、`Lids/Normal2` 的 `*_cover_*.png` 在导入时按源文件原样复制，保留 alpha、透明区 RGB 和尺寸；导入前资源及 SHA256 清单保存在 `DevData/Backups/FA-Import-20260921`。后续统一尺寸见上节。
...
证据位于 `DevData/Backups/FA-Import-20260921/`：`manifest.json`、`static-validation.log`、`runtime-checks.txt`、`Player.log`、`directory-audit.json`。目录审计结果为 `affected_candidates=0`、`scan_errors=0`；测试游戏仅位于 `%LOCALAPPDATA%/RimWorldModTests/Mugirl/FA-Import-20260921-Game`，未在模组内创建联接。
...
- `1.6/FacialAnimation/Patches/Mugirl_FacialAnimation.xml` 给 `Mugirl` 追加 FA comp。当前使用这些层：
...
Mugirl 当前 TypeDef 全部写了 `raceName=Mugirl`。每层都有 `Normal` 到 `Normal7` 七套贴图目录，其中 `Normal` 对应原 1 号脸，`Normal2` 到 `Normal7` 对应 `DevData/Assets/FA/2` 到 `DevData/Assets/FA/7`：

| 层 | TypeDef | 贴图根路径 |
| --- | --- | --- |
| 头 | `Mugirl_HeadNormal`、`Mugirl_HeadNormal2`...`7` | `FA/Heads_Blank/Normal`、`Normal2`...`7` |
| 眼睛 | `Mugirl_EyeNormal`、`Mugirl_EyeNormal2`...`7` | `FA/Eyes/Normal`、`Normal2`...`7` |
| 眼皮 | `Mugirl_LidNormal`、`Mugirl_LidNormal2`...`7` | `FA/Lids/Normal`、`Normal2`...`7` |
| 眉毛 | `Mugirl_BrowNormal`、`Mugirl_BrowNormal2`...`7` | `FA/Brows/Normal`、`Normal2`...`7` |
| 嘴 | `Mugirl_MouthNormal`、`Mugirl_MouthNormal2`...`7` | `FA/Mouth/Normal`、`Normal2`...`7` |
| 眼皮附加层 | `Mugirl_LidOptionNormal`、`Mugirl_LidOptionNormal2`...`7` | `FA/LidOptions/Normal`、`Normal2`...`7` |
| 情绪叠层 | `Mugirl_EmotionNormal`、`Mugirl_EmotionNormal2`...`7` | `FA/Emotions/Normal`、`Normal2`...`7` |

`raceName` 的行为很重要：只要某个种族存在任意专属 TypeDef，FA 就用该种族的专属 TypeDef 池替代通用池。Mugirl 因此不会随机到 FA 自带的人类脸型。
...
性别目录来自 `Pawn.gender` 的枚举名，例如 `Female`、`Male`、`None`。雪牛娘种族 `maleGenderProbability=0`，当前只维护 `Female` 贴图是有意选择。若调试或其他 mod 生成非女性 Mugirl，FA 会找不到对应性别贴图并显示透明 dummy 或回退。
...
- 它启用了 `SkinControllerComp`，这是和 Mugirl 当前补丁最明显的功能差异。
...
- 大多数 TypeDef 写了 `enableUnisexTexPath=True`，所以贴图末级目录是 `Unisex`，不是 `Female` / `Male`。Mugirl 当前没有启用该选项，所以仍应使用 `Female`。
...
- `LidShapeDef close`、`sleep1`、`sleep2`、`cry` 写了 `disableEyeball=true`，用于闭眼或哭泣时隐藏眼球。Mugirl 新增完全闭眼形状时也可以采用这个规则。
...
FA 为 Pawn 建立动画池时有一个种族过滤规则：只要当前种族存在任何专属 `FaceAnimationDef.raceName`，该 Pawn 的动画池就只使用该种族的专属动画，不再混用 FA 自带通用动画。Mugirl 已有完整 `raceName=Mugirl` 动画集，因此新增常驻或 Job 动画也必须写 `raceName=Mugirl`。
...
`normal_Mugirl` 是整个动画系统的底座：它常驻、priority 为 0，并提供完整基础 shape。不要删除它，也不要给它加 mood、pain、thought 或 Job 限制。否则合成帧可能缺少基础 shape，部分控制器会出现空引用风险。
...
FA 会在渲染前移除原版 head draw request，用 FA 的空白 head 和各部件重新拼脸。Mugirl 当前图层顺序大致是：
...
- `sad` 嘴型不只是低心情嘴，也表示严肃、紧绷但没有咬牙的表情。`AttackStatic_Mugirl` 和 `WaitCombat_Mugirl` 使用 `sad`；`tight` 是咬牙/憋力嘴，应保留给近战、劳动、挖矿、吃饭咀嚼等更用力的场景。
- 疼痛动画不需要哭泣层。`painHigh_Mugirl`、`painExtreme_Mugirl` 用痛嘴、眉毛、汗等表现即可；`tear`、`Mugirl_cry` 主要留给低心情、倒地或 Lovin 后段。
...
| `Mouth` | `sad`、`smile`、`surprise`、`tight`、`Mugirl_lovin` | 已接入 | Mugirl ShapeDef 提供；心情、疼痛、工作、Lovin、穿脱衣等动画引用。`sad` 也用于严肃嘴，例如 `AttackStatic` 和 `Wait_Combat`；`tight` 表示咬牙切齿或明显用力。 |
...
10. 游戏内验证至少生成一名普通成年女性 Mugirl，检查南/东朝向、眨眼、心情低落、疼痛、倒地、穿衣/脱衣、Lovin、吃饭和战斗待机。
...
FA 改动后的 fresh log 不应出现 Mugirl 或 FacialAnimation 相关红字。若游戏内看不到动画，先排除这些设置因素：
