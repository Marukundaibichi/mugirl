...
该参数现在只重新生成三张身体贴图，原图备份写入 `DevData/Backups/AppearanceTextureRepair-20260921/OriginalTextures`。
...
| 隔离运行 | 环境与结果 |
| --- | --- |
| `DevData/Evidence/StylingStartup-WithoutYaOpt1` | Harmony、全部 DLC、HAR、Mugirl：18 PASS / 0 FAIL |
| `DevData/Evidence/StylingStartup-WithYaOpt1` | 加入 YaOpt，未加入 Prepatcher：18 PASS / 0 FAIL；此轮日志显示延迟纹理加载未启用，不作为该优化路径的验证 |
| `DevData/Evidence/StylingStartup-WithYaOpt3` | Prepatcher、Harmony、全部 DLC、YaOpt、HAR、Mugirl：19 PASS / 0 FAIL；Prepatcher 已实际完成程序集重载，YaOpt 正常启用 |
