...
- 加入事件、任务、派系关系、快递分支。
- R18 内容常驻，不再提供设置按钮、过滤、贸易库存剔除或地图/角色清理。
- 玩家可见文本的含义；迁移为翻译键时不得静默改变原文案语义。
...
- 删除 `enableAdultContent` 设置字段和设置窗口 checkbox。
- 删除 `AdultContentUtility` 运行时门禁；R18 内容不再经过 enable/filter/cleanup 策略判断。
- 旧的成人内容清理、交易库存过滤和穿戴物品删除逻辑不再保留。
- 保留 `AdultContentExtension` 与 `CompProperties_AdultContentControl` 作为 XML 元数据和 Def 加载兼容外壳，但不参与运行时过滤。
