...
`Start-StylingStartupValidation.ps1` 为每次运行新建独立 `TMP/StylingStartup-*` 配置，支持 `-WithYaOpt`；驱动检查真实图标、Harmony Prefix/Postfix 数量及重复初始化后状态，随后退出，不创建或读取玩家世界。
测试结束后需普通 Release Rebuild，排除测试驱动。实际运行结果在完成后记录。
