...
进入任务地图后，在地图中央生成 `Mugirl_CorporateFusionResearchSite` 研究站蓝图。蓝图正式占地先清除冲突建筑、植物与 Filth，并保留原有地板；仅当某座建筑所在格缺少所需承载力时，才在该建筑实际占地补混凝土支撑，因此仍能适应沙地、泥地和水面等地貌。Prefab 生成后会把现场所有耗燃料设施填满，确保奶发电机立即供电。外围按圆角距离和 Perlin 噪声清除山岩、冲突建筑与岩顶，保留原有地表和植被形成自然渐变边缘。四面通往地图边缘的通道会轻微弯曲并随机变化宽度，只在原地表不可通行时补为土壤，保证研究站不会嵌进山体且玩家能够进入交互。随后触发研究团队联络。对话完成前双方处于可交谈状态，目标不会因为临时用了敌对派系而在对白前主动开火。
...
| 拒绝执行，通知巨企退出 | 终止清剿，研究团队保持友善并表达感谢 | 放弃巨企报酬，巨企好感 -60；投资者致谢，支线以保护研究团队的结果结束 |
| 暂不决定 | 暂停对话，任务保留现场选择入口 | 不领取奖励、不自动拒绝，期限继续 |

拒绝分支的投资者感谢以对白和后续信件为基础；可建议赠送少量研究成果，首轮价值 500–1,000，但不能把拒绝分支做成两边收取完整奖励的漏洞。
...
- 工程与内容维护：[维护指南](maintenance-guide.md)、[内容更新指南](content-update-guide.md)、[本地化规范](localization-and-comments.md)。
